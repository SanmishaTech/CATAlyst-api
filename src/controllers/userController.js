...
const createUser = async (req, res, next) => {
  ...
  const schema = z.object({
    ...
    clientId: z.number().optional(), // For users, specify which client they belong to
    active: z.boolean().optional(),
  }).superRefine(async (data, ctx) => {
    // Validate role hierarchy: only admin can create clients, only admin/client can create users
    if (data.role === "client" && req.user.role !== "admin") {
      ctx.addIssue({
        path: ["role"],
