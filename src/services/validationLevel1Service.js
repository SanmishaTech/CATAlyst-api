...
const processBatchValidation = async (batchId) => {
  try {
    ...
    if (batch.validation_1_status !== null) {
    ...
    console.log(
      `[Validation] Validating ${orders.length} orders for batch ${batchId}`
    );
    ...
    console.log(
      `[Validation] Batch ${batchId} completed: ${successCount} passed, ${failCount} failed - Overall: ${allPassed ? "PASSED" : "FAILED"}`
    );
...
const processExecutionBatchValidation = async (batchId, batch = null) => {
  try {
    ...
    console.log(
      `[Validation] Validating ${executions.length} executions for batch ${batchId}`
    );
    ...
    console.log(
      `[Validation] Execution Batch ${batchId} completed: ${successCount} passed, ${failCount} failed - Overall: ${allPassed ? "PASSED" : "FAILED"}`
    );
