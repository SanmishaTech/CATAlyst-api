...
const createClientAndUser = async () => {
  const client = await prisma.client.create({
    data: {
      name: `synthetic-exe-v2-${makeScenarioId("client")}`,
      catEnabled: false,
      sixZeroFiveEnabled: false,
      loprEnabled: false,
      active: true,
      exe_validation_2: defaultValidation2ExecutionConditions,
    },
    select: { id: true, name: true },
  });

  const email = `synthetic-exe-v2-${makeScenarioId("user")}@example.com`.toLowerCase();
  const user = await prisma.user.create({
    data: {
      name: "Synthetic Execution V2",
      email,
      password: "test",
      role: "USER",
      clientId: client.id,
...
const deriveFailOverrides = (schemaField, rule, baseExecution) => {
  ...
  if (cond.includes("lookup table") || cond.includes("nano") || cond.includes("throughout")) {
    return {
      ...
      overrides: {},
      expectedEnforced: false,
      note: "Rule text not machine-parseable by current engine",
    };
  }
...
const main = async () => {
  ...
  const batch = await prisma.batch.create({
    data: {
      userId: user.id,
      status: "pending",
      fileName: "scenario_exe_v2_all_fields_synthetic",
      fileType: "execution",
      validation_1: true,
  ...
  console.log("\n[coverExecutionValidation2AllFieldsSynthetic] Synthetic fixtures:");
  console.table([
    {
      clientId: client.id,
      userId: user.id,
  ...
  console.log(
    `\n[coverExecutionValidation2AllFieldsSynthetic] Per-field results (${rows.length} rows):`
  );
  console.table(rows.slice(0, topN));

  if (rows.length > topN) {
    console.log(
      `\n[coverExecutionValidation2AllFieldsSynthetic] Note: output truncated to --limit ${topN}. Re-run with --limit ${rows.length} to see all.`
    );
  }
  ...
  if (notWorking.length) {
    console.log(
      "\n[coverExecutionValidation2AllFieldsSynthetic] Expected-to-be-enforced rules that did NOT produce errors (investigate):"
    );
  ...
  if (unsupportedRows.length) {
    console.log(
      "\n[coverExecutionValidation2AllFieldsSynthetic] Rules marked unsupported/not-parseable by current generator:"
    );
    console.table(unsupportedRows.slice(0, topN));
  }

  console.log("\n[coverExecutionValidation2AllFieldsSynthetic] Done");
};
...
main()
  ...
  .catch(async (err) => {
    console.error("\n[coverExecutionValidation2AllFieldsSynthetic] Error:");
    console.error(err);
    try {
      await prisma.$disconnect();
    } catch (_) {}
